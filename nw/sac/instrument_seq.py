        ...
        if line.find("sac.h") != -1:
            ...
            if _while == nth_while:
                if time_nest:
                    ...
                    outfile.write("printf(\"%f\\n\", runtime);\n");              
            _while = _while + 1;
        elif line.find("SAC_WL_STRIDE_LOOP0_BEGIN(0") != -1 and _goto > nth_goto:
            if time_nest == 1:
                outfile.write("gettimeofday( &nest_start, NULL);\n");  
                outfile.write(line);
            else:
                outfile.write(line);
        elif line.find("SAC_WL_STRIDE_LOOP_END(0") != -1 and _goto > nth_goto:
            if time_nest == 1:
                outfile.write(line);
...
def count_forloop_nests( source=""):
    ...
    infile = open( source, "r");  
    for line in infile:
        if line.find("SAC_WL_STRIDE_LOOP0_BEGIN(0") != -1 and _goto > nth_goto:
