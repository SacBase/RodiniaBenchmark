...
standard_flags = ["-DV=1", "-DV=2", "-DV=3"];
runtime_csv = ["./runtimes/cuda_manual_unoptimised.txt", "./runtimes/cuda_manual_memopt.txt","./runtimes/cuda_manual_memopt+lao.txt"];
...
while r < len(standard_flags):
    i = 0;
    while i < len(sizes):
	cmd = "nvcc $nvccopt " + standard_flags[r]  + " -DSIZE=" + `sizes[i]` + " -DITER=" + `iterations` + " " +  prog + " -o " + out_exe;
	print cmd;
	os.system(cmd); 
