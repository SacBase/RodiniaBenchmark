	    ...
	    int    *block_deltas) 
{
  ...
  __shared__ int deltas[THREADS_PER_BLOCK]; // 256
  if(threadIdx.x < THREADS_PER_BLOCK) {
     deltas[threadIdx.x] = 0;  // initialization
  }
#endif
  if (point_id < npoints) {
#ifdef GPU_DELTA_REDUCTION
  ...
  if(threadIdx.x < 1) { // the first thread in the block
  ...
  __shared__ int new_center_ids[THREADS_PER_BLOCK]; // 256
