...
{	
  num_blocks = npoints / num_threads; // npoints / 256
  ...
  /* allocate memory for membership_new[] and initialize to -1 (host) */
		   ...
		   )
{
  ...
  /* copy back membership (device to host) This will be the new membership and 
   * it will later be compared with the old membership */
  cudaMemcpy(membership_new, membership_d, npoints*sizeof(int), cudaMemcpyDeviceToHost);	

#ifdef BLOCK_CENTER_REDUCE
  /*** Copy back arrays of per block sums ***/
  float * block_clusters_h = (float *) malloc( num_blocks_perdim * num_blocks_perdim * nclusters * nfeatures * sizeof(float));
        
  cudaMemcpy(block_clusters_h, block_clusters_d, num_blocks_perdim * num_blocks_perdim *  nclusters * nfeatures * sizeof(float), cudaMemcpyDeviceToHost);
#endif
