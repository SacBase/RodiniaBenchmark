...
if flops:
    i = 0;
    while i < len(sizes):
	cmd = "sac2c -v0 -O3 -d cccall -DSEQ -DSIZE=" + `sizes[i]` + " -DITER=" + `iterations` + " " +  prog + " -o " + out_exe;
	print cmd;
	os.system(cmd); 
	...
	os.system( out_sac2c);
        ...
        os.system( "./" + out_exe + " < ../input/sac_" + `sizes[i]` + ".txt >> " + flops_file);
...
standard_flags = ["-DSEQ", "-t cuda -nomemopt", "-t cuda"];
