...
texture <double> tex_CDF;
texture <double> tex_sums;
...
__device__ double d_roundDouble(double value)
{
  int newValue = (int)(value);
  if(value - newValue < 0.5) {
    return newValue;
...
//    likelihoodSum += (pow((double)(I[ind[index*numOnes + x]] - 100),2) - pow((double)(I[ind[index*numOnes + x]]-228),2))/50.0;
    likelihoodSum += ((I[ind[index*numOnes + x]] - 100)*(I[ind[index*numOnes + x]] - 100) - 
                      (I[ind[index*numOnes + x]] - 228)*(I[ind[index*numOnes + x]] - 228))/50.0;
  }
...
__global__ void sum_kernel(double* partial_sums, int Nparticles)
{
  ...
  if(i == 0) {
    int x;
    double sum = 0;
    for(x = 0; x < Nparticles/threads_per_block; x++) {
...
__global__ void likelihood_kernel(double * arrayX, double * arrayY, double * CDF, int * ind, int * objxy, double * likelihood, int * I, double * u, double * weights, int Nparticles, int countOnes, int max_size, int k, int IszY, int Nfr, int *seed, double * partial_sums)
{
  ...
  if(i < Nparticles) {
    for(y = 0; y < countOnes; y++) {
      indX = d_roundDouble(arrayX[i]) + objxy[y*2 + 1];
      indY = d_roundDouble(arrayY[i]) + objxy[y*2];
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles)
{
  ...
  for(k = 1; k < Nfr; k++) {
    ...
    /* After kernel sum_kernel, the first element of array partial_sums stores
     * the sum if all weights (equal to the variable sumWieght in the OpenMP
     * implementation). The partial_sums array is initialized in the kernel
     * likelihood_kernel. */
