...
double roundDouble(double value)
{
  ...
  if(value - newValue < 0.5) {
    ...
    /* Note here that the orginal code is written as
     * newValue++. However, this is incorrect as the 
     * return value is without being incremented by
     * one. This is a BUG!!!*/
    return ++newValue;
  }
}
...
void dilate_matrix(int * matrix, int posX, int posY, int posZ, int dimX, int dimY, int dimZ, int error)
{
  ...
  for(x = startX; x < endX; x++){
    for(y = startY; y < endY; y++){
      ...
      double distance = sqrt( (double)(x-posX)*(double)(x-posX) + (double)(y-posY)*(double)(y-posY));
      if(distance < error) {
...
void particleFilter(int * I, int IszX, int IszY, int Nfr, int * seed, int Nparticles)
{
  ...
  int radius = 5;
  int diameter = radius*2 - 1; // i.e. 9
  int *disk = (int *)malloc(diameter*diameter*sizeof(int)); // 9*9
  ...
  for(x = 0; x < Nparticles; x++){
    weights[x] = 1/((double)(Nparticles)); // 1/10000
  }
  ...
  for(k = 1; k < Nfr; k++) {
    ...
    for(x = 0; x < Nparticles; x++) {
      ...
      //calc ind
#ifdef ORIGINAL /* The orginal implementation distributed with Rodinia */
...
#else /* The two for loops can actually be fused */
      likelihood[x] = 0;
      for(y = 0; y < countOnes; y++) {
	indX = (int)(roundDouble(arrayX[x])) + (int)(objxy[y*2 + 1]);
	indY = (int)(roundDouble(arrayY[x])) + (int)(objxy[y*2]);
	...
	likelihood[x] += ((I[ind[x*countOnes + y]] - 100)*(I[ind[x*countOnes + y]] - 100) - 
                          (I[ind[x*countOnes + y]]-228)*(I[ind[x*countOnes + y]]-228))/50.0;
      }
#endif
  ...
  /* In the orginal Rodinia implementation, this loop is parallelized with
   * OpenMP pragma. However, it seems that this parallelization leads to 
   * different resutls when compared to the sequential execution of the 
   * same loop. It appears that this loop is parallelizable. As to why
   * parallelization leads to different result, further investigation is
   * needed. For the time being this loop is left as sequential unless
   * the OPENMP pragma is defined.  
   */
#ifdef OPENMP 
