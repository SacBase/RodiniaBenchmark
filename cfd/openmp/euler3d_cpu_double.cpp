...
#define NVAR (VAR_DENSITY_ENERGY+1)            /* 5 */
...
inline void compute_flux_contribution( double& density, double3& momentum, double& density_energy, double& pressure, double3& velocity, 
                                       double3& fc_momentum_x, double3& fc_momentum_y, double3& fc_momentum_z, double3& fc_density_energy)
{
...
void compute_flux(int nelr, int* elements_surrounding_elements, double* normals, double* variables, double* fluxes)
{
  ...
  for(int i = 0; i < nelr; i++) {
    ...
    double3 velocity_i;             				 
    compute_velocity(density_i, momentum_i, velocity_i); // velocity = momentum / density;
...
int main(int argc, char** argv)
{
  ...
  // set far field conditions
  {
    ...
    ff_variable[VAR_DENSITY] = double(1.4); /* ff_variable[0] = double(1.4) */
    ...
    ff_variable[VAR_MOMENTUM+0] = ff_variable[VAR_DENSITY] * ff_velocity.x; /* ff_variable[1] */
    ff_variable[VAR_MOMENTUM+1] = ff_variable[VAR_DENSITY] * ff_velocity.y; /* ff_variable[2] */
    ff_variable[VAR_MOMENTUM+2] = ff_variable[VAR_DENSITY] * ff_velocity.z; /* ff_variable[3] */

    ff_variable[VAR_DENSITY_ENERGY] = ff_variable[VAR_DENSITY]*(double(0.5)*(ff_speed*ff_speed)) + 
	                              (ff_pressure / double(GAMMA-1.0)); /* ff_variable[4] */
  ...
  // read data from files into arrays areas, elements_surrounding_elements and normals 
  {
